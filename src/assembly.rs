...
//! mutate peer/trust/capability/evidence/citizenship/execution state directly.
...
fn is_assembly_member_id(value: &str) -> bool {
    ...
    tail.bytes().enumerate().all(|(index, byte)| {
        let head_ok = byte.is_ascii_lowercase() || byte.is_ascii_digit();
        if index == 0 {
            head_ok
        } else {
            head_ok || matches!(byte, b'.' | b'_' | b'-')
        }
    })
}
...
fn validate_text(value: &str, maximum_bytes: usize, error: &'static str) -> Result<(), AssemblyError> {
    if value.is_empty() || value.len() > maximum_bytes {
        return Err(AssemblyError(error.into()));
    }
    Ok(())
}
...
impl FederationAssembly {
    ...
    pub fn admit_member(
        ...
        if self.representation_subjects.contains(&subject) {
            return Err(AssemblyError("assembly_representation_subject_already_active".into()));
        }
    ...
    pub fn submit_proposal(
        ...
        match draft.kind {
            ...
            {
                return Err(AssemblyError("assembly_advisory_compatibility_invalid".into()));
            }
            AssemblyProposalKind::ForkProposal
                if draft.compatibility != ProtocolCompatibility::Breaking =>
            {
                return Err(AssemblyError("assembly_fork_must_be_breaking".into()));
            }
            _ => {}
        }
        ...
        let identity_projection = serde_json::json!({
            ...
            "sequence": sequence,
            "draft": &draft,
            "charter_gate": &charter_gate,
            "electorate_member_ids": &electorate,
        });
        let proposal_id = canonical_id(&identity_projection)?;
        let status = if charter_gate.current_lineage_eligible || draft.kind == AssemblyProposalKind::ForkProposal {
    ...
    pub fn attach_advisory(
        ...
        if !is_sha256_ref(&report_ref)
            || source_node.as_deref().is_some_and(|value| !is_node_id(value))
        {
        ...
        if !matches!(proposal.status, AssemblyProposalStatus::Open | AssemblyProposalStatus::ForkRequired) {
            return Err(AssemblyError("assembly_proposal_not_open_for_advisory".into()));
        }
    ...
    pub fn cast_vote(
        ...
        if !proposal.electorate_member_ids.iter().any(|eligible| eligible == member_id) {
            return Err(AssemblyError("assembly_voter_not_in_electorate_snapshot".into()));
        }
        if proposal.votes.insert(member_id.to_string(), choice).is_some() {
    ...
    pub fn withdraw_proposal(
        ...
        if proposal.draft.proposer_member_id != proposer_member_id
            || !matches!(proposal.status, AssemblyProposalStatus::Open | AssemblyProposalStatus::ForkRequired)
        {
    ...
    pub fn finalize(&mut self, proposal_id: &str) -> Result<AssemblyGovernanceReceipt, AssemblyError> {
        ...
        let (outcome, status) = match snapshot.status {
            AssemblyProposalStatus::ForkRequired => {
                (GovernanceOutcome::ForkRequired, AssemblyProposalStatus::ForkRequired)
            }
            AssemblyProposalStatus::Withdrawn => {
                (GovernanceOutcome::Withdrawn, AssemblyProposalStatus::Withdrawn)
            }
            AssemblyProposalStatus::Open if quorum_met && approval_met => {
                if snapshot.draft.kind == AssemblyProposalKind::ForkProposal {
                    (GovernanceOutcome::ForkEndorsed, AssemblyProposalStatus::ForkEndorsed)
                } else {
                    (GovernanceOutcome::Accepted, AssemblyProposalStatus::Accepted)
                }
            }
            AssemblyProposalStatus::Open => {
                (GovernanceOutcome::Rejected, AssemblyProposalStatus::Rejected)
            }
        ...
        let version_path = match (snapshot.draft.kind, snapshot.draft.compatibility, outcome) {
            ...
            (AssemblyProposalKind::ProtocolAmendment, ProtocolCompatibility::BackwardCompatible, _) => {
        ...
        let source_change_required = matches!(
            outcome,
            GovernanceOutcome::Accepted | GovernanceOutcome::ForkRequired | GovernanceOutcome::ForkEndorsed
...
mod tests {
    ...
    fn draft(proposer: &str, kind: AssemblyProposalKind) -> AssemblyProposalDraft {
        AssemblyProposalDraft {
            ...
            summary: "Clarify an interoperable protocol rule without changing member-local authority.".into(),
            kind,
    ...
    fn three_member_assembly() -> FederationAssembly {
        ...
        assembly
            .admit_member(member("assembly:alpha", "Principal Alpha", Some("fed:qsol:alpha")))
            .unwrap();
        assembly
            .admit_member(member("assembly:beta", "Principal Beta", None))
            .unwrap();
        assembly
            .admit_member(member("assembly:gamma", "Principal Gamma", Some("fed:qsol:gamma")))
            .unwrap();
        assembly
    }
    ...
    fn network_membership_does_not_grant_assembly_membership() {
        ...
        let proposal = assembly
            .submit_proposal(draft("assembly:alpha", AssemblyProposalKind::ProtocolAmendment))
            .unwrap();
        assert!(assembly
            .cast_vote(&proposal.proposal_id, "fed:qsol:network-only-peer", VoteChoice::Yes)
            .is_err());
    ...
    fn nexus_advisory_report_has_zero_vote_weight() {
        let mut assembly = three_member_assembly();
        let proposal = assembly
            .submit_proposal(draft("assembly:alpha", AssemblyProposalKind::AdvisoryResolution))
            .unwrap();
