...
use crate::{
    ...
    MAX_SIGNED_MESSAGE_LIFETIME_SECONDS,
};
...
pub const MAX_CAPABILITY_ADVERTISEMENT_LIFETIME_SECONDS: i64 = MAX_SIGNED_MESSAGE_LIFETIME_SECONDS;
...
impl PeerRegistry {
    pub fn open(root: impl AsRef<Path>) -> Result<Self, PeeringError> {
        ...
        fs::create_dir_all(root)
            .map_err(|error| PeeringError(format!("peer_root_create:{error}")))?;
        let root = fs::canonicalize(root)
            .map_err(|error| PeeringError(format!("peer_root_canonicalize:{error}")))?;
    ...
    pub fn state(&self, node_id: &str) -> Result<PeerStateView, PeeringError> {
        Ok(self
            ...
            .unwrap_or(PeerStateView::Unknown))
    }
    ...
    pub fn introduce(
        ...
        if observed_snapshot
            .as_deref()
            .is_some_and(|value| !crate::wire::is_sha256_ref(value))
        {
        ...
        if let Some(mut old) = self.get(&node_id)? {
            ...
            if canonical_struct(&old.identity)? != canonical_struct(&identity)? {
                return Err(PeeringError("peer_initial_identity_immutable".into()));
            }
    ...
    /// Import verified peer identity as quarantined only when it is new locally.
    /// Existing local lifecycle/admission state is never demoted by an archival import.
    pub fn import_quarantined(
        ...
        let existed = self.get(&identity.node_id)?.is_some();
        ...
        self.transition(
            &identity.node_id,
            PeerLifecycleState::Quarantined,
            updated_at,
        )?;
        Ok(RegistryWriteDisposition::Applied)
    }
    ...
    pub fn attach_archival_capability_advertisement(
        ...
        let mut record = self
            .get(node_id)?
            .ok_or_else(|| PeeringError("capability_peer_unknown".into()))?;
        ...
        if let Some(old) = &record.capability_advertisement {
            ...
            if advertisement.sequence == old.sequence {
                if canonical_struct(old)? == canonical_struct(&advertisement)? {
                    return Ok(());
                }
    ...
    pub fn transition(
        &self,
        node_id: &str,
        target: PeerLifecycleState,
        updated_at: &str,
    ) -> Result<(), PeeringError> {
        if !valid_timestamp(updated_at) {
            return Err(PeeringError("peer_transition_time_invalid".into()));
        }
        let mut record = self
            .get(node_id)?
            .ok_or_else(|| PeeringError("peer_unknown".into()))?;
        if !transition_allowed(record.state, target) {
            return Err(PeeringError("peer_transition_forbidden".into()));
        }
    ...
    pub fn disconnect(
        ...
        let mut record = self
            .get(node_id)?
            .ok_or_else(|| PeeringError("peer_unknown".into()))?;
        if record.state == PeerLifecycleState::Revoked
    ...
    pub fn propose_rejoin(
        &self,
        node_id: &str,
        remote_snapshot: &str,
    ) -> Result<RejoinDisposition, PeeringError> {
        if !crate::wire::is_sha256_ref(remote_snapshot) {
            return Err(PeeringError("rejoin_snapshot_invalid".into()));
        }
        let record = self
            .get(node_id)?
            .ok_or_else(|| PeeringError("peer_unknown".into()))?;
        if record.state != PeerLifecycleState::Disconnected {
            return Err(PeeringError("rejoin_requires_disconnected_peer".into()));
        }
    ...
    pub fn confirm_rejoin(
        ...
        if !valid_timestamp(updated_at) {
            return Err(PeeringError("rejoin_time_invalid".into()));
        }
        let disposition = self.propose_rejoin(node_id, remote_snapshot)?;
        if disposition == RejoinDisposition::ExplicitReconciliationRequired
        ...
        let target = record
            ...
            .unwrap_or(PeerLifecycleState::Introduced);
    ...
    pub fn record_capability_advertisement(
        ...
        if !valid_timestamp(updated_at) {
            return Err(PeeringError("capability_update_time_invalid".into()));
        }
        let mut record = self
        ...
        if let Some(old) = &record.capability_advertisement {
            ...
            if advertisement.sequence == old.sequence {
                if canonical_struct(&advertisement)? == canonical_struct(old)? {
                    return Ok(RegistryWriteDisposition::Duplicate);
                }
                return Err(PeeringError("capability_same_sequence_divergence".into()));
            }
...
impl TrustRegistry {
    pub fn open(path: impl AsRef<Path>) -> Result<Self, PeeringError> {
        ...
        let entries = if path.exists() {
            ...
            require_exact_canonical(&raw)?;
            let snapshot: TrustSnapshot = serde_json::from_slice(&raw)
                .map_err(|error| PeeringError(format!("trust_schema:{error}")))?;
            if snapshot.schema != TRUST_REGISTRY_SCHEMA_V1
                || snapshot.entries.keys().any(|node| !is_node_id(node))
            {
    ...
    pub fn get(&self, node_id: &str) -> LocalTrustLevel {
        self.entries
            .get(node_id)
            .copied()
            .unwrap_or(LocalTrustLevel::Unknown)
    }

    pub fn set(&mut self, node_id: &str, level: LocalTrustLevel) -> Result<(), PeeringError> {
        if !is_node_id(node_id) {
            return Err(PeeringError("trust_node_invalid".into()));
        }
        let mut staged = self.entries.clone();
        if level == LocalTrustLevel::Unknown {
            staged.remove(node_id);
        } else {
            staged.insert(node_id.into(), level);
        }
        persist_trust_entries(&self.path, &staged)?;
        self.entries = staged;
        Ok(())
    }
}

fn persist_trust_entries(
    path: &Path,
    entries: &BTreeMap<String, LocalTrustLevel>,
) -> Result<(), PeeringError> {
    atomic_replace(
        path,
        &canonical_struct(&TrustSnapshot {
            schema: TRUST_REGISTRY_SCHEMA_V1.into(),
...
impl LocalCapabilityPolicy {
    pub fn open(path: impl AsRef<Path>) -> Result<Self, PeeringError> {
        ...
        let entries = if path.exists() {
            ...
            require_exact_canonical(&raw)?;
            let snapshot: CapabilityPolicySnapshot = serde_json::from_slice(&raw)
                .map_err(|error| PeeringError(format!("policy_schema:{error}")))?;
            if snapshot.schema != CAPABILITY_POLICY_SCHEMA_V1 {
            ...
            for (node, capabilities) in &snapshot.entries {
                if !is_node_id(node)
                    || capabilities
                        .keys()
                        .any(|capability| !is_capability_id(capability))
                {
                    return Err(PeeringError("capability_policy_corrupt".into()));
                }
            }
    ...
    pub fn decision(&self, node_id: &str, capability: &str) -> CapabilityDecision {
        self.entries
            .get(node_id)
            .and_then(|values| values.get(capability))
            .copied()
    ...
    pub fn set(
        ...
        if !is_node_id(node_id) || !is_capability_id(capability) {
            return Err(PeeringError("capability_policy_key_invalid".into()));
        }
        let mut staged = self.entries.clone();
        staged
            .entry(node_id.into())
            .or_default()
            .insert(capability.into(), decision);
        persist_capability_entries(&self.path, &staged)?;
        self.entries = staged;
        Ok(())
    }

    pub fn advertised_and_allowed(
        &self,
        peer: &PeerRecord,
        capability: &str,
        now_unix: i64,
    ) -> bool {
        if peer.state != PeerLifecycleState::Admitted {
            return false;
        }
        let Some(advertisement) = &peer.capability_advertisement else {
            return false;
        };
        if advertisement_is_active(advertisement, now_unix).is_err() {
            return false;
        }
        advertisement
            .capabilities
...
fn persist_capability_entries(
    path: &Path,
    entries: &BTreeMap<String, BTreeMap<String, CapabilityDecision>>,
) -> Result<(), PeeringError> {
    atomic_replace(
        path,
        &canonical_struct(&CapabilityPolicySnapshot {
            schema: CAPABILITY_POLICY_SCHEMA_V1.into(),
            entries: entries.clone(),
        })?,
    )
}
...
pub fn create_capability_advertisement(
    ...
    if sequence == 0
        || expires <= issued
        || expires - issued > MAX_CAPABILITY_ADVERTISEMENT_LIFETIME_SECONDS
    {
    ...
    envelope.message_id =
        derive_message_id(&envelope_bytes).map_err(|error| PeeringError(error.0))?;
    let proof =
        sign_envelope(identity, signing_key, envelope).map_err(|error| PeeringError(error.0))?;
...
pub fn verify_capability_advertisement_signature(
    ...
    if advertisement.node_id != identity.node_id {
        return Err(PeeringError("capability_node_mismatch".into()));
    }
    ...
    if assessment.signature == SignatureValidity::Valid {
        ...
        Err(PeeringError("capability_signature_invalid".into()))
    }
}

fn advertisement_is_active(
    advertisement: &CapabilityAdvertisement,
    now_unix: i64,
) -> Result<(), PeeringError> {
    let issued = parse_timestamp(&advertisement.issued_at)?;
    ...
    if now_unix < issued || now_unix > expires {
        return Err(PeeringError("capability_advertisement_inactive".into()));
    }
    Ok(())
}
...
fn validate_capability_set(capabilities: &[String]) -> Result<(), PeeringError> {
    if capabilities.len() > 128 {
        return Err(PeeringError("capability_advertisement_too_large".into()));
    }
    let mut seen = HashSet::new();
    if capabilities
        .iter()
        .any(|value| !is_capability_id(value) || !seen.insert(value.as_str()))
    {
...
pub fn rebuild_peer_identity(
    identity: &NodeIdentityDocument,
    lifecycle: &[PeerLifecycleRecord],
) -> Result<IdentityState, PeeringError> {
    let mut state = IdentityState::from_document(identity)
        .map_err(|error| PeeringError(format!("peer_identity:{error}")))?;
    for record in lifecycle {
        match record {
            PeerLifecycleRecord::Rotation(value) => state
                .apply_rotation(value)
                .map_err(|error| PeeringError(format!("peer_rotation:{error}")))?,
            PeerLifecycleRecord::Status(value) => state
                .apply_key_status(value)
                .map_err(|error| PeeringError(format!("peer_status:{error}")))?,
        }
    }
    Ok(state)
}

fn require_lifecycle_prefix(
    old: &[PeerLifecycleRecord],
    candidate: &[PeerLifecycleRecord],
) -> Result<(), PeeringError> {
    if candidate.len() < old.len() {
        return Err(PeeringError("peer_lifecycle_prefix_rewrite_forbidden".into()));
    }
    for (index, old_record) in old.iter().enumerate() {
        if canonical_struct(old_record)? != canonical_struct(&candidate[index])? {
            return Err(PeeringError("peer_lifecycle_prefix_rewrite_forbidden".into()));
        }
    }
    Ok(())
...
fn identity_fingerprint(
    ...
    #[derive(Serialize)]
    struct Snapshot<'a> {
        identity: &'a NodeIdentityDocument,
        lifecycle: &'a [PeerLifecycleRecord],
    }
    object_id(&canonical_struct(&Snapshot { identity, lifecycle })?)
        .map_err(|error| PeeringError(error.0))
}
...
fn validate_peer_record(record: &PeerRecord) -> Result<(), PeeringError> {
    ...
    if record.state == PeerLifecycleState::Disconnected {
        if record.pre_disconnect_state.is_none() || record.partition_snapshot.is_none() {
            return Err(PeeringError("peer_disconnected_state_incomplete".into()));
        }
    }
    let rebuilt = rebuild_peer_identity(&record.identity, &record.lifecycle)?;
    if rebuilt.sequence != record.lifecycle_sequence
        || identity_fingerprint(&record.identity, &record.lifecycle)? != record.identity_fingerprint
    {
        return Err(PeeringError("peer_record_identity_drift".into()));
    }
    if let Some(advertisement) = &record.capability_advertisement {
        verify_capability_advertisement_signature(advertisement, &rebuilt)?;
    }
    Ok(())
}
...
fn transition_allowed(from: PeerLifecycleState, to: PeerLifecycleState) -> bool {
    ...
    match from {
        PeerLifecycleState::Introduced => matches!(
            to,
            PeerLifecycleState::Admitted
                | PeerLifecycleState::Quarantined
                | PeerLifecycleState::Revoked
                | PeerLifecycleState::Disconnected
        ),
        PeerLifecycleState::Admitted => matches!(
            to,
            PeerLifecycleState::Quarantined
                | PeerLifecycleState::Revoked
                | PeerLifecycleState::Disconnected
        ),
        PeerLifecycleState::Quarantined => matches!(
            to,
            PeerLifecycleState::Admitted
                | PeerLifecycleState::Revoked
                | PeerLifecycleState::Disconnected
...
fn parse_timestamp(value: &str) -> Result<i64, PeeringError> {
    if !valid_timestamp(value) {
        return Err(PeeringError("peer_timestamp_invalid".into()));
    }
    DateTime::parse_from_rfc3339(value)
        .map(|value| value.timestamp())
        .map_err(|_| PeeringError("peer_timestamp_invalid".into()))
}

fn canonical_struct<T: Serialize>(value: &T) -> Result<Vec<u8>, PeeringError> {
    canonicalize(
        &serde_json::to_vec(value).map_err(|error| PeeringError(format!("peer_encode:{error}")))?,
    )
    .map_err(|error| PeeringError(error.0))
}

fn require_exact_canonical(raw: &[u8]) -> Result<(), PeeringError> {
    if canonicalize(raw).map_err(|error| PeeringError(error.0))? == raw {
        Ok(())
    } else {
        Err(PeeringError("peer_registry_bytes_not_canonical".into()))
    }
}
...
fn prepare_file_path(path: &Path) -> Result<PathBuf, PeeringError> {
    ...
    fs::create_dir_all(parent)
        .map_err(|error| PeeringError(format!("registry_parent_create:{error}")))?;
    let canonical_parent = fs::canonicalize(parent)
        .map_err(|error| PeeringError(format!("registry_parent_canonicalize:{error}")))?;
    let name = path
        .file_name()
        .ok_or_else(|| PeeringError("registry_filename_missing".into()))?;
...
fn atomic_replace(path: &Path, bytes: &[u8]) -> Result<(), PeeringError> {
    let name = path
        ...
        .ok_or_else(|| PeeringError("registry_filename_invalid".into()))?;
    ...
    let mut file = OpenOptions::new()
        ...
        .write(true)
        .open(&temporary)
        .map_err(|error| PeeringError(format!("registry_temp_open:{error}")))?;
    file.write_all(bytes)
        .map_err(|error| PeeringError(format!("registry_write:{error}")))?;
    file.flush()
        .map_err(|error| PeeringError(format!("registry_flush:{error}")))?;
    file.sync_all()
        .map_err(|error| PeeringError(format!("registry_fsync:{error}")))?;
    drop(file);
    fs::rename(&temporary, path)
        .map_err(|error| PeeringError(format!("registry_rename:{error}")))?;
    sync_parent(path)
}

#[cfg(unix)]
fn sync_parent(path: &Path) -> Result<(), PeeringError> {
    File::open(path.parent().unwrap_or_else(|| Path::new(".")))
        .map_err(|error| PeeringError(format!("registry_parent_open:{error}")))?
        .sync_all()
        .map_err(|error| PeeringError(format!("registry_parent_fsync:{error}")))
}

#[cfg(not(unix))]
fn sync_parent(_path: &Path) -> Result<(), PeeringError> {
    Err(PeeringError(
        "registry_parent_fsync_unsupported_platform".into(),
    ))
}
...
mod tests {
    ...
    fn peer_lifecycle_and_trust_are_separate_and_durable() {
        ...
        peers
            .introduce(doc.clone(), vec![], None, "2026-08-23T00:00:01Z")
            .unwrap();
        peers
            .transition(
                &doc.node_id,
        ...
        assert_eq!(
            TrustRegistry::open(root.join("trust.json"))
                .unwrap()
                .get(&doc.node_id),
            LocalTrustLevel::Trusted
        );
        assert_eq!(
            PeerRegistry::open(root.join("peer-registry"))
                .unwrap()
                .state(&doc.node_id)
                .unwrap(),
            PeerStateView::Admitted
        );
        let _ = fs::remove_dir_all(root);
    }

    #[test]
    fn capability_advertisement_requires_local_admission_and_policy() {
        ...
        peers
            .introduce(doc.clone(), vec![], None, "2026-08-23T00:00:01Z")
            .unwrap();
        let ad = create_capability_advertisement(
            &state,
            &op,
            1,
        ...
        .unwrap();
        peers
            .record_capability_advertisement(ad, 1_787_443_320, "2026-08-23T00:00:02Z")
            .unwrap();
        policy
            .set(
                &doc.node_id,
            ...
            .unwrap();
        let introduced = peers.get(&doc.node_id).unwrap().unwrap();
        assert!(!policy.advertised_and_allowed(
            &introduced,
            "evidence.exchange/1",
            1_787_443_320
        ));
        peers
            ...
            .unwrap();
        let admitted = peers.get(&doc.node_id).unwrap().unwrap();
        assert!(policy.advertised_and_allowed(
            &admitted,
            "evidence.exchange/1",
            1_787_443_320
        ...
        peers
            ...
            .unwrap();
        let revoked = peers.get(&doc.node_id).unwrap().unwrap();
        assert!(!policy.advertised_and_allowed(
            &revoked,
            "evidence.exchange/1",
            1_787_443_320
    ...
    #[test]
    fn advertisement_cannot_outlive_phase2_signed_proof() {
        let (_, op, _, state) = identity();
        assert!(create_capability_advertisement(
            &state,
            &op,
            1,
            "2026-08-23T00:00:00Z",
    ...
    fn partition_rejoin_never_silently_reconciles() {
        ...
        peers
            .disconnect(&doc.node_id, &old, "2026-08-23T00:01:00Z")
            .unwrap();
        assert_eq!(
            peers.propose_rejoin(&doc.node_id, &changed).unwrap(),
            RejoinDisposition::ExplicitReconciliationRequired
        );
        assert!(peers
            .confirm_rejoin(
    ...
    fn lifecycle_replay_and_rollback_fail_across_restart() {
        ...
        let recovery = state
            .create_recovery_rotation(&root_key, &next, "2026-08-23T00:11:00Z")
            .unwrap();
        peers
            .introduce(
                doc.clone(),
                vec![
                    PeerLifecycleRecord::Status(status),
                    PeerLifecycleRecord::Rotation(recovery),
                ],
                None,
                "2026-08-23T00:12:00Z",
            )
            .unwrap();
        drop(peers);
        let reopened = PeerRegistry::open(&root).unwrap();
        assert!(reopened
            .introduce(doc, vec![], None, "2026-08-23T00:13:00Z")
            .is_err());
        let _ = fs::remove_dir_all(root);
    }

    #[test]
    fn lifecycle_advancement_must_preserve_exact_prefix() {
        let root = temp_root("prefix");
        let peers = PeerRegistry::open(&root).unwrap();
        let (root_key, op, doc, base) = identity();
        let old_status = base
            .create_key_status_record(
                &root_key,
                &op.key_id(),
        ...
        peers
            .introduce(
                doc.clone(),
                vec![PeerLifecycleRecord::Status(old_status)],
                None,
                "2026-08-23T00:10:01Z",
            )
            .unwrap();

        let mut alternate = IdentityState::from_document(&doc).unwrap();
        let alternate_status = alternate
            .create_key_status_record(
                &root_key,
                &op.key_id(),
                KeyStatusKind::Compromised,
                "2026-08-23T00:10:30Z",
                KeyStatusReason::ConfirmedCompromise,
            )
            .unwrap();
        alternate.apply_key_status(&alternate_status).unwrap();
        let next = LocalSigningKey::from_seed_hex(NEXT).unwrap();
        let recovery = alternate
            .create_recovery_rotation(&root_key, &next, "2026-08-23T00:11:00Z")
            .unwrap();
        let error = peers
            .introduce(
                doc,
                vec![
                    PeerLifecycleRecord::Status(alternate_status),
                    PeerLifecycleRecord::Rotation(recovery),
                ],
                None,
                "2026-08-23T00:12:00Z",
            ...
            .unwrap_err();
        assert_eq!(error.0, "peer_lifecycle_prefix_rewrite_forbidden");
        let _ = fs::remove_dir_all(root);
    }

    #[test]
    fn disconnected_snapshot_cannot_be_replaced_by_lifecycle_update() {
        let root = temp_root("snapshot");
        let peers = PeerRegistry::open(&root).unwrap();
        let (root_key, op, doc, state) = identity();
        peers
            .introduce(doc.clone(), vec![], None, "2026-08-23T00:00:01Z")
            .unwrap();
        peers
            ...
            .unwrap();
        let snapshot_a = format!("sha256:{}", "a".repeat(64));
        let snapshot_b = format!("sha256:{}", "b".repeat(64));
        peers
            .disconnect(&doc.node_id, &snapshot_a, "2026-08-23T00:01:00Z")
            .unwrap();
        let status = state
            .create_key_status_record(
                &root_key,
        ...
        peers
            .introduce(
                doc.clone(),
                vec![PeerLifecycleRecord::Status(status)],
                Some(snapshot_b.clone()),
                "2026-08-23T00:02:01Z",
            )
            .unwrap();
        assert_eq!(
            peers.propose_rejoin(&doc.node_id, &snapshot_b).unwrap(),
            RejoinDisposition::ExplicitReconciliationRequired
        );
        assert_eq!(
            peers.get(&doc.node_id).unwrap().unwrap().partition_snapshot,
            Some(snapshot_a)
        );
        let _ = fs::remove_dir_all(root);
    }
    ...
    #[test]
    fn import_does_not_demote_existing_admitted_peer() {
        let root = temp_root("import-preserve");
        let peers = PeerRegistry::open(&root).unwrap();
        let (_, _, doc, _) = identity();
        peers
            .introduce(doc.clone(), vec![], None, "2026-08-23T00:00:01Z")
        ...
        assert_eq!(
            peers
                .import_quarantined(doc.clone(), vec![], "2026-08-23T00:00:03Z")
                .unwrap(),
            RegistryWriteDisposition::Duplicate
        );
        assert_eq!(peers.state(&doc.node_id).unwrap(), PeerStateView::Admitted);
        let _ = fs::remove_dir_all(root);
    }
    ...
    #[test]
    fn failed_trust_and_policy_persistence_do_not_change_live_state() {
        let root = temp_root("transactional");
        fs::create_dir_all(&root).unwrap();
        let (_, _, doc, _) = identity();

        let trust_path = root.join("trust.json");
        let mut trust = TrustRegistry::open(&trust_path).unwrap();
        fs::create_dir_all(&trust_path).unwrap();
        assert!(trust.set(&doc.node_id, LocalTrustLevel::Trusted).is_err());
        assert_eq!(trust.get(&doc.node_id), LocalTrustLevel::Unknown);

        let policy_path = root.join("policy.json");
        let mut policy = LocalCapabilityPolicy::open(&policy_path).unwrap();
        ...
        assert!(policy
            ...
            .is_err());
        assert_eq!(
            policy.decision(&doc.node_id, "evidence.exchange/1"),
