...
use crate::crypto::{sign_payload_domain, verify_payload_domain};
use crate::wire::{is_capability_id, is_node_id, is_wire_timestamp};
use crate::{
    IdentityState, KeyRotationRecord, KeyStatusRecord, LocalSigningKey, NodeIdentityDocument,
...
pub const CAPABILITY_ADVERTISEMENT_DOMAIN: &[u8] = b"qsol-fed-capability-advertisement/1\0";
...
impl PeerRegistry {
    ...
    pub fn identity_state(&self, node_id: &str) -> Result<Option<IdentityState>, PeeringError> {
        ...
        Ok(Some(rebuild_identity(&record.identity, &record.lifecycle)?))
    }
    ...
    pub fn introduce(
        ...
        let identity_state = rebuild_identity(&identity, &lifecycle)?;
        let fingerprint = identity_fingerprint(&identity, &lifecycle)?;
        let node_id = identity.node_id.clone();
        let existing = self.get(&node_id)?;
    ...
    pub fn import_quarantined(
        ...
        self.introduce(identity.clone(), lifecycle, None, updated_at)?;
    ...
    pub fn disconnect(
        ...
        if !crate::wire::is_sha256_ref(local_snapshot) {
            return Err(PeeringError("partition_snapshot_invalid".into()));
        }
        let mut record = self.get(node_id)?.ok_or_else(|| PeeringError("peer_unknown".into()))?;
    ...
    pub fn record_capability_advertisement(
        ...
        let mut record = self.get(&advertisement.node_id)?
            .ok_or_else(|| PeeringError("capability_peer_unknown".into()))?;
        let identity = rebuild_identity(&record.identity, &record.lifecycle)?;
...
impl TrustRegistry {
    ...
    pub fn set(&mut self, node_id: &str, level: LocalTrustLevel) -> Result<(), PeeringError> {
        ...
        if level == LocalTrustLevel::Unknown {
            ...
            self.entries.insert(node_id.into(), level);
        }
        self.persist()
    }

    fn persist(&self) -> Result<(), PeeringError> {
        atomic_replace(&self.path, &canonical_struct(&TrustSnapshot {
...
impl LocalCapabilityPolicy {
    ...
    fn persist(&self) -> Result<(), PeeringError> {
        atomic_replace(&self.path, &canonical_struct(&CapabilityPolicySnapshot {
...
pub fn create_capability_advertisement(
    ...
    if signing_key.key_id() != identity.current_key_id {
        return Err(PeeringError("capability_signing_key_not_current".into()));
    }
    validate_capability_set(&capabilities)?;
    capabilities.sort();
    let mut advertisement = CapabilityAdvertisement {
    ...
    advertisement.signature = sign_payload_domain(signing_key, CAPABILITY_ADVERTISEMENT_DOMAIN, &advertisement_payload(&advertisement)?);
    Ok(advertisement)
}
...
pub fn verify_capability_advertisement(
    ...
    let key = identity.operational_key(&advertisement.key_id)
        .ok_or_else(|| PeeringError("capability_key_unknown".into()))?;
    if key.status != OperationalKeyStatus::Active
        || issued < key.valid_from
        || key.valid_until.is_some_and(|until| issued > until)
    {
        return Err(PeeringError("capability_key_not_valid_at_issue".into()));
    }
    if !verify_payload_domain(
        &key.public_key,
        CAPABILITY_ADVERTISEMENT_DOMAIN,
        &advertisement_payload(advertisement)?,
        &advertisement.signature,
    ).map_err(|error| PeeringError(error.0))? {
        return Err(PeeringError("capability_signature_invalid".into()));
    }
...
fn validate_advertisement_shape(advertisement: &CapabilityAdvertisement) -> Result<(), PeeringError> {
    if advertisement.schema != CAPABILITY_ADVERTISEMENT_SCHEMA_V1
        ...
        || advertisement.signature.len() != 128
        || !advertisement.signature.bytes().all(|value| value.is_ascii_hexdigit() && !value.is_ascii_uppercase())
    {
...
fn advertisement_payload(advertisement: &CapabilityAdvertisement) -> Result<Vec<u8>, PeeringError> {
    canonical_struct(&CapabilityAdvertisementPayload {
        ...
        key_id: &advertisement.key_id,
    })
}

fn rebuild_identity(identity: &NodeIdentityDocument, lifecycle: &[PeerLifecycleRecord]) -> Result<IdentityState, PeeringError> {
    let mut state = IdentityState::from_document(identity)
        .map_err(|error| PeeringError(format!("peer_identity:{error}")))?;
    for record in lifecycle {
        match record {
            PeerLifecycleRecord::Rotation(value) => state.apply_rotation(value)
                .map_err(|error| PeeringError(format!("peer_rotation:{error}")))?,
            PeerLifecycleRecord::Status(value) => state.apply_key_status(value)
...
fn identity_fingerprint(identity: &NodeIdentityDocument, lifecycle: &[PeerLifecycleRecord]) -> Result<String, PeeringError> {
    ...
    let raw = canonical_struct(&Snapshot { identity, lifecycle })?;
    object_id(&raw).map_err(|error| PeeringError(error.0))
}
...
fn validate_peer_record(record: &PeerRecord) -> Result<(), PeeringError> {
    ...
    let rebuilt = rebuild_identity(&record.identity, &record.lifecycle)?;
...
fn transition_allowed(from: PeerLifecycleState, to: PeerLifecycleState) -> bool {
    ...
    match from {
        ...
        PeerLifecycleState::Revoked => false,
        PeerLifecycleState::Disconnected => false,
    }
}
...
fn canonical_struct<T: Serialize>(value: &T) -> Result<Vec<u8>, PeeringError> {
    let raw = serde_json::to_vec(value).map_err(|error| PeeringError(format!("peer_encode:{error}")))?;
    canonicalize(&raw).map_err(|error| PeeringError(error.0))
}

fn require_exact_canonical(raw: &[u8]) -> Result<(), PeeringError> {
    let canonical = canonicalize(raw).map_err(|error| PeeringError(error.0))?;
    if canonical == raw {
        Ok(())
    } else {
...
fn sync_parent(path: &Path) -> Result<(), PeeringError> {
    ...
    directory.sync_all().map_err(|error| PeeringError(format!("registry_parent_fsync:{error}")))
}

#[cfg(not(unix))]
fn sync_parent(_path: &Path) -> Result<(), PeeringError> {
    Err(PeeringError("registry_parent_fsync_unsupported_platform".into()))
}
...
mod tests {
    ...
    fn peer_lifecycle_and_trust_are_separate_and_durable() {
        ...
        let trust2 = TrustRegistry::open(root.join("trust.json")).unwrap();
        assert_eq!(trust2.get(&doc.node_id), LocalTrustLevel::Trusted);
    ...
    fn lifecycle_replay_and_rollback_fail_across_restart() {
        ...
        peers.introduce(doc.clone(), vec![PeerLifecycleRecord::Status(status.clone()), PeerLifecycleRecord::Rotation(recovery.clone())], None, "2026-08-23T00:12:00Z").unwrap();
        drop(peers);
