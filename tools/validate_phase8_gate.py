...
def validate_contract() -> None:
    ...
    for key in ("message_id_preserved_across_transports", "payload_ref_preserved_across_transports", "provenance_ref_preserved_across_transports", "transport_acceptance_requires_signature_valid", "transport_acceptance_requires_current_identity", "transport_acceptance_requires_fresh_replay_state", "transport_acceptance_requires_local_peer_admission"):
    ...
    require(nat["ticket_is_route_hint_only"] is True and nat["ticket_node_must_match_authenticated_sender"] is True, "NAT identity binding drift")
    for key in ("ticket_grants_trust", "ticket_grants_authority", "candidate_may_embed_credentials", "identity_weakening_allowed"):
    ...
    for key in ("every_hop_preserves_frame_id", "every_hop_preserves_message_id", "every_hop_preserves_payload_ref", "every_hop_links_previous_receipt"):
...
def validate_schemas_and_source() -> None:
    ...
    for key in ("identity_weakened", "authority_promoted", "provenance_lost", "resource_bound_breached", "holodeck_invariant_drift"):
        require(drill["properties"][key].get("const") is False, f"drill hard-false schema drift: {key}")
...
def main() -> None:
    ...
    print("phase8 transport-resilience gate OK: five bounded reference profiles preserve authenticated identity and original message/provenance identity; NAT and relay metadata remain non-authoritative; key-compromise/resource/partition/archive drills pass; and Holodeck sandbox invariants remain transport-independent")
