...
def validate_docs_and_ci() -> None:
    ...
    require('/usr/local/qsol-moriarty-rust-toolchain/bin/cargo test --all-targets --locked' in workflow, "CI Rust suite does not use the pre-execution snapshot")
    require("MORIARTY_RUST_TOOLCHAIN_ROOT: /usr/local/qsol-moriarty-rust-toolchain" in workflow, "CI MORIARTY Rust snapshot binding missing")
