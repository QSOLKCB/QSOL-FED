...
Run III additionally recorded restart recovery of the prior Council/session and AGENT-X seat identity from persisted world state. The retained in-repository extract preserves only the aggregate adversarial result: **12/12 recorded adversarial mutation attempts were rejected fail-closed**. It does not retain per-probe IDs, mutated fields, expected rejection conditions, or individual outcomes, so this record does not characterize which authority/evidence/identity boundaries those twelve attempts exercised. The restart inspection was read-only; this record does not infer a resumed post-restart voting authority state from that inspection.
...
`README4AI.md` registers the core and supplemental empirical layer in the machine-facing inventory and normative precedence, including the retained supplements and their validator, while preserving the bounded assurance summary and validation command list. Phase 10 remains historically frozen by checking the published Phase-10-era `README4AI.md` at the tested pre-assurance commit while the current Phase 10 AI-manifest fields are semantically revalidated against their proof boundaries.
