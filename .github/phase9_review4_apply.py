...
# 1. Close repository-controlled Cargo configuration around rust_all.
run_path = Path("tools/run_moriarty.py")
source = run_path.read_text(encoding="utf-8")
if "moriarty_cargo_project_config_forbidden" not in source:
    anchor = '    package = manifest.get("package")\n'
    insert = '''    cargo_config_root = source_root / ".cargo"
    for config_name in ("config", "config.toml"):
        ...
        fail(f"moriarty_cargo_project_config_forbidden:{config_name}")
'''
    source = replace_once(source, anchor, insert + anchor, "Cargo project config closure")

# 2. Make disposable probe cleanup survive chmod(0) trees without buffering entries.
if "def _force_remove_probe_path(" not in source:
    start = source.index("def _cleanup_probe_writable_paths(*paths: Path) -> None:\n")
    end = source.index("\ndef _run_probe_with_cleanup(", start)
    cleanup = '''def _force_remove_probe_path(path: Path) -> None:
...
'''
    source = source[:start] + cleanup + source[end:]
run_path.write_text(source, encoding="utf-8")


# 3. Reject root/set-ID/capability-bearing launchers before constructing a probe child.
isolation_path = Path("tools/moriarty_isolation.py")
source = isolation_path.read_text(encoding="utf-8")
if "def _proc_status_unprivileged(" not in source:
    marker = "def probe_isolation_preexec(\n"
    pos = source.index(marker)
    helper = '''def _proc_status_unprivileged(status_text: str) -> bool:
    fields: dict[str, str] = {}
...
'''
    ...
    sig = source.index(marker, pos + len(helper))
    body = source.index("): \n", sig) if False else -1
    signature_end = source.index("):\n", sig) + 3
    source = source[:signature_end] + "    require_unprivileged_probe_launcher()\n" + source[signature_end:]
isolation_path.write_text(source, encoding="utf-8")


# 4. Encode the report semantics that external JSON Schema consumers can enforce.
...
replay_item["allOf"] = [
    ...
    },
    {
        ...
        "then": {"properties": {"failure_kind": {"enum": [
            "target_failure_not_reproduced", "resolution_probe_not_green", "replay_setup_error"
        ]}}},
    },
    {
    ...
    },
    {
        ...
        "then": {"properties": {
            "ok": {"const": False},
            "target_reproduced": {"const": False},
            "failure_result": {"type": "object"},
        }},
    },
    {
...
schema["allOf"] = [
    ...
    {
        "if": {"properties": {"graduated": {"const": False}}, "required": ["graduated"]},
        "then": {"anyOf": [
            {"properties": {"unresolved_counterexamples": {"minimum": 1}}},
            {"properties": {"probe_results": {"contains": {"properties": {"ok": {"const": False}}, "required": ["ok"]}}}},
            {"properties": {"remediation_replays": {"contains": {"properties": {"ok": {"const": False}}, "required": ["ok"]}}}},
            {"properties": {"counterexamples": {"contains": {"properties": {"status": {"const": "unresolved"}}, "required": ["status"]}}}},
        ]},
    },
]
...
# Freeze the new contract and add negative regressions to the executable gate.
validator_path = Path("tools/validate_phase9_gate.py")
source = validator_path.read_text(encoding="utf-8")
if "report probe semantic conditional set drift" not in source:
    anchor = '    require(set(result_props["failure_kind"].get("enum", [])) == {None, "exit_nonzero", "timeout", "tool_error"}, "report failure-kind schema drift")\n'
    block = '''    result_item_schema = report_props["probe_results"]["items"]
...
'''
    source = replace_once(source, anchor, anchor + block, "report schema conditional assertions")

if "repository Cargo config" not in source:
    anchor = '        _expect_reject(lambda: moriarty.validate_rust_target_topology(fake), "Cargo target topology override")\n'
    block = '''        (fake / "Cargo.toml").write_text('[package]\\nname="qsol-fed"\\nversion="0.0.0"\\nedition="2024"\\n', encoding="utf-8")
        (fake / ".cargo").mkdir()
...
'''
    source = replace_once(source, anchor, anchor + block, "Cargo config negative test")

if "probe cleanup could not reclaim chmod(0) tree" not in source:
    anchor = '    with tempfile.TemporaryDirectory(prefix="moriarty-python-shadow-") as temp_dir:\n'
    block = '''    cleanup_root = Path(os.environ["MORIARTY_PROBE_WRITABLE_ROOT"]).resolve(strict=True) / "cleanup-permission-negative"
...
'''
    source = replace_once(source, anchor, block + anchor, "cleanup negative test")

if "canonical CI launcher is privileged" not in source:
    anchor = '    isolation = moriarty._moriarty_isolation\n'
    block = '''    safe_status = "Uid:\\t1000\\t1000\\t1000\\t1000\\nGid:\\t1000\\t1000\\t1000\\t1000\\nCapInh:\\t0000000000000000\\nCapPrm:\\t0000000000000000\\nCapEff:\\t0000000000000000\\nCapAmb:\\t0000000000000000\\n"
...
'''
    source = replace_once(source, anchor, anchor + block, "privileged launcher negative tests")
